...
class WordTrigger(Trigger):
    ...
    def isWordIn(self, word, story):
        if word in puncSplit(story):
            return True
        else:
            return False
...
class AndTrigger(Trigger):
    ...
    def evaluate(self, story):
        return self.one.evaluate(story) and self.another.evaluate(story)
...
class OrTrigger(Trigger):
    def __init__(self, one, another):
        ...
        self.another = another
    def evaluate(self, story):
        return self.one.evaluate(story) or self.another.evaluate(story)

class PhraseTrigger(Trigger):
    def __init__(self, phrase):
        self.phrase = phrase
    def evaluate(self, story):
        if self.phrase in story.getSummary() or \
...
def filterStories(stories, triggerlist):
    ...
    for item in triggerlist:
        for story in stories:
            if item.evaluate(story) and story not in res:
                res.append(story)
    return res 
